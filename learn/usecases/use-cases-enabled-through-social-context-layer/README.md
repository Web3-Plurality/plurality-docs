# 🤝 Use cases enabled through Social Context Layer

Plurality's open social context layer contains portable contexts against a wallet address. It allows creation of interoperable, self-custodial smart profiles against a user's wallet address. The context is aggregated from various web2 and web3 platforms.&#x20;

On top of open social context layer, many different use cases can be developed.&#x20;

This section discusses a few use cases where plurality could be used, but don't let this hinder your creativity.&#x20;

{% hint style="info" %}
If your dApp idea is not on this list, [contact us](../../contact-us.md) via your preferred channel. We will be happy to jam with you on your idea!
{% endhint %}

Some of the plurality use cases are:

1. [Easy Login](easy-login.md)
2. [Social feed personalization](personalization.md)
3. [Personalized Marketplace listings](marketplace-listings.md)
4. [Content Recommendations](content-recommendations.md)
5. [Relevant jobs on gig platforms](relevant-jobs-on-gig-platforms.md)
6.
7. [Sybil Resistance](sybil-resistance.md)
8. [Web3 Gaming](web3-gaming.md)
